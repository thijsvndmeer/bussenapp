...
content = re.sub(r"const PlayerHandModal = React\.lazy\(\(\) => import\('\./components/modals/PlayerHandModal'\)\.then\(m => \(\{ default: m\.PlayerHandModal \}\)\)\);", "import { PlayerHandModal } from './components/modals/PlayerHandModal';", content)
