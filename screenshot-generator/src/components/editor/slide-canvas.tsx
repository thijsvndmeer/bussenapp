...
function getDefaultRects(
  ...
  switch (layout) {
    ...
    case "three-devices":
      return {
        caption: { x: cW * 0.08, y: cH * 0.04, width: capW, height: capH, align: "center" },
        deviceSecondary: {
          x: -cW * 0.1,
          y: cH - smallH * 0.85 - cH * 0.04,
          width: smallW * 0.85,
...
export function SlideCanvas({
  ...
  function renderDevice(id: "device" | "deviceSecondary" | "deviceTertiary", rect: Rect, src: string, extraStyle?: React.CSSProperties) {
    const saved = slide.transforms?.[id];
    const rotation = saved?.rotation ?? 0;
    const zIndex = saved?.zIndex ?? (id === "deviceSecondary" ? 2 : id === "deviceTertiary" ? 1 : 3);
    return (
      <Movable
        rect={rect}
        cW={cW}
