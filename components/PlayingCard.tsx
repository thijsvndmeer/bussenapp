...
const PlayingCard: React.FC<PlayingCardProps> = ({
  ...
  const isNeonGlass = style === CardStyle.NEON_GLASS;

  const redColor = isNeonGlass ? 'text-rose-400' : isDark ? 'text-red-500' : isClassic ? 'text-[#c21807]' : 'text-[#e11d48]';
  const blackColor = isNeonGlass ? 'text-cyan-400' : isDark ? 'text-slate-200' : isClassic ? 'text-[#000000]' : 'text-[#1e293b]';
  const textColor = isRed ? redColor : blackColor;

  const glowClass = isNeonGlass ? (isRed ? 'drop-shadow-[0_0_8px_rgba(251,113,133,0.8)]' : 'drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]') : isDark ? (isRed ? 'drop-shadow-[0_0_4px_rgba(239,68,68,0.4)]' : 'drop-shadow-[0_0_4px_rgba(255,255,255,0.2)]') : '';
  ...
  return (
    <div
      className={`perspective-1000 ${sizeConfig.width} aspect-[1/1.4] ${sizeConfig.radius} ${className} relative select-none group
        ...
        ${isClassic ? 'font-serif' : isNeonGlass ? 'font-mono' : 'font-sans'}
      `}
    ...
    >
      ...
      `}>
        ...
        <div className={`
          ...
          ${isDark ? 'bg-slate-950 border-slate-800' : isClassic ? 'bg-[#fffdf5] border-[#dcd0b9]' : isNeonGlass ? 'bg-gradient-to-br from-slate-900/40 to-slate-800/40 backdrop-blur-xl border-white/20' : 'bg-gradient-to-b from-[#fdfdfd] to-[#f3f4f6] border-white/80'}
          ${sizeConfig.radius}
          border
          ${!isNeonGlass ? 'ring-1 ring-black/5 shadow-[0_16px_40px_-14px_rgba(0,0,0,0.65)]' : 'shadow-[0_8px_32px_rgba(0,0,0,0.3)]'}
          overflow-hidden
        `}>
          {/* Texture Overlay */}
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-visible">
                ...
                {!isAce && !isFaceCard && (
                  <div className="w-[65%] h-[75%] relative pointer-events-none">
                    {size === 'sm' ? (
                      <div className={`w-full h-full flex items-center justify-center font-serif font-black text-3xl ${isDark || isNeonGlass ? 'opacity-40' : 'opacity-20'} tracking-tighter ${textColor}`}>
                        {rankLabel}
                      </div>
                    ) : (
                      pipContent
        ...
        <div className={`
          ...
          ${isDark ? 'bg-slate-950' : isClassic ? 'bg-red-700' : isNeonGlass ? 'bg-gradient-to-br from-slate-900/40 to-slate-800/40 backdrop-blur-xl' : 'bg-[#1e40af]'}
          ${sizeConfig.radius}
          ${(isNeonGlass || isDark) ? 'border border-white/20' : (isClassic ? 'border-[4px]' : 'border-[6px]') + ' border-white'}
        ...
        `}>
          ...
          {/* specialized Neon Glass back visuals */}
          {isNeonGlass && (
            <>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className={`w-1/2 h-1/3 border-2 ${isNeonGlass ? 'border-cyan-400/30 shadow-[0_0_15px_rgba(34,211,238,0.2)]' : 'border-white/30'} rounded-full flex items-center justify-center ${isNeonGlass ? 'bg-white/5 backdrop-blur-md' : 'backdrop-blur-[1px]'}`}>
              <div className={`text-white font-sans font-black italic tracking-widest transform -rotate-12 text-sm ${isNeonGlass ? 'drop-shadow-[0_0_12px_rgba(255,255,255,0.9)]' : 'drop-shadow-md'}`}>
                BUSSEN
              </div>
            </div>
          </div>

          {/* Corner Decals for Neon Glass */}
          {isNeonGlass && (
            <>
