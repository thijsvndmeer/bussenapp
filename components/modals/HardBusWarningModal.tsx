...
export const HardBusWarningModal: React.FC<HardBusWarningModalProps> = React.memo(({
  ...
  return (
    <SlideMenuModal
      isOpen={isOpen}
      onClose={onCancel}
      className="bg-slate-900 border-2 border-amber-500/50 rounded-3xl p-6 sm:p-8 shadow-[0_0_50px_rgba(245,158,11,0.3)] w-full max-w-md m-4 space-y-6 text-center relative"
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            <button
              ...
              className="flex-1 py-3.5 px-4 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 font-black transition-all active:scale-95 text-sm uppercase tracking-wider cursor-pointer"
            >
              {t('Aanpassen')}
            </button>
            <button
              ...
              className="flex-1 py-3.5 px-4 rounded-xl border border-slate-700 hover:bg-slate-800 text-slate-300 font-bold transition-all active:scale-95 text-sm uppercase tracking-wider cursor-pointer"
            >
              {t('Toch Starten')}
            </button>
          </div>
