...
export const PlayerHandModal: React.FC<PlayerHandModalProps> = React.memo(({
  ...
  return (
    <div
      className="fixed inset-0 z-[120] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in"
      onClick={onClose}
    >
      <div
        className="w-full max-w-sm m-4 relative bg-slate-900 border border-white/10 rounded-3xl shadow-2xl p-6 text-center animate-in zoom-in-95"
      ...
      >
        ...
        <button
          onClick={onClose}
          className="w-full bg-amber-500 text-slate-900 font-black py-4 rounded-2xl hover:bg-amber-400 active:scale-95 transition-all uppercase tracking-widest"
        >
          {t('Sluiten')}
        </button>
      </div>
