      ...
      <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'max-h-96' : 'max-h-0'}`}>
        <div className="p-4 space-y-4 bg-black/20 border-t border-slate-800">
          {sliders.map(({ key, label, min, max }) => (
            <div key={key}>
              ...
              <input
                ...
                onChange={(e) => setDraftValues((current) => ({ ...current, [key]: parseInt(e.target.value, 10) }))}
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <button onClick={() => toggleSetting('sharedBus')} className={`w-12 h-6 rounded-full relative transition-all ${settings.sharedBus ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'bg-slate-700'}`}>
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <button onClick={() => toggleSetting('doublePyramidCards')} className={`w-12 h-6 rounded-full relative transition-all ${settings.doublePyramidCards ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'bg-slate-700'}`}>
